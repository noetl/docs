...
- Replay validation now starts with state-report integrity: `scripts/check_replay_state_report.py --report replay.json` requires the canonical replay envelope, enforces `checksum_algorithm=sha256`, and recomputes the replay state's top-level checksum plus projection checksum bundles from the replay JSON. Mutated state, incomplete envelope data, malformed snapshot metadata, or surface-level drift fail before parity or payload gates run.
