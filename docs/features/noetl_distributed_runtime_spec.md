...
- Payload-resolution validation now treats resolved payload checksums as part of the replay contract: `scripts/check_replay_payload_resolution_report.py` requires lowercase SHA-256 hex values for every resolved payload checksum before accepting the summary.
