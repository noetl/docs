...
- Replay validation now starts with state-report integrity: `scripts/check_replay_state_report.py --report replay.json` requires the canonical replay envelope, validates required string fields plus non-negative integer fields, enforces `checksum_algorithm=sha256`, and recomputes the replay state's top-level checksum plus projection checksum bundles from the replay JSON. Mutated state, incomplete envelope data, malformed field types or snapshot metadata, or surface-level drift fail before parity or payload gates run.
- Replay parity validation now treats checksum shape as part of the contract: `scripts/check_replay_parity_report.py` requires lowercase SHA-256 hex values for replayed and live bundles unless `--allow-invalid-checksum-shape` is used for a legacy ad-hoc report.
- Payload-resolution validation now treats row shape and resolved payload checksums as part of the replay contract: `scripts/check_replay_payload_resolution_report.py` requires every `payload_resolution` entry and its `resolution` value to be objects, then requires lowercase SHA-256 hex values for every resolved payload checksum before accepting the summary.
