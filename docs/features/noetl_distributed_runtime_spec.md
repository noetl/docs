...
- Replay validation now starts with state-report integrity: `scripts/check_replay_state_report.py --report replay.json` recomputes the replay state's top-level checksum and projection checksum bundles from the replay JSON, rejecting mutated state, malformed snapshot metadata, or surface-level drift before parity or payload gates run.
