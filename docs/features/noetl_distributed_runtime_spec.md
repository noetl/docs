...
- Replay validation now starts with state-report integrity: `scripts/check_replay_state_report.py --report replay.json` recomputes projection checksum bundles from the replay JSON and rejects malformed or inconsistent snapshot metadata before parity or payload gates run.
