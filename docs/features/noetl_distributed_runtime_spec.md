...
- Latest local kind validation for `noetl/noetl#435`: image `local/noetl:2026-05-16-22-39`, health/replay/pod/log smokes passed, `/metrics` exposes the IPC counters, a temporary frame claim emitted `stream_version=1`, 64-character `envelope_checksum`, and non-null `catalog_id`, in-pod Arrow IPC TempStore round-trip passed, in-pod cursor-worker frame capture produced two Arrow-backed frames from three claimed rows, and the deployed server image imports `ReplayStateProjector`, `NATSProjectorWorker`, and the `python -m noetl.projector` entrypoint.
