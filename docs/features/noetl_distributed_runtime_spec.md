...
- `noetl/noetl#468` applies the boundary to execution routes: `/api/execute` initial `command.issued` rows and `/api/executions/{execution_id}/cancel` `execution.cancelled` rows enqueue before commit, then drain committed outbox rows. Command recovery publish remains separate because it is worker dispatch, not projector/event distribution.
